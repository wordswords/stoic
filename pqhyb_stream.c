...
#define MAGIC "PQHS01\0"
#define MAGIC_LEN 8
#define RSA_SEED_LEN 32
#define AES_KEY_LEN 32
#define GCM_IV_LEN 12
#define GCM_TAG_LEN 16
#define IO_CHUNK (4 * 1024 * 1024)
...
static void write_u64_be(FILE *f, uint64_t v) {
    unsigned char b[8];
    for (int i = 0; i < 8; i++) b[7 - i] = (unsigned char)((v >> (8 * i)) & 0xff);
    if (fwrite(b, 1, 8, f) != 8) die_msg("write_u64_be failed");
}

static uint64_t read_u64_be(FILE *f) {
    unsigned char b[8];
    if (fread(b, 1, 8, f) != 8) die_msg("read_u64_be failed");
    uint64_t v = 0;
...
static void gcm_decrypt_stream(FILE *in, FILE *out,
                               const unsigned char key[AES_KEY_LEN],
                               const unsigned char iv[GCM_IV_LEN],
                               const unsigned char *aad, size_t aadlen,
                               uint64_t cipher_len,
                               const unsigned char tag[GCM_TAG_LEN],
                               uint64_t expected_plain_len) {
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    unsigned char *inbuf = OPENSSL_malloc(IO_CHUNK);
    unsigned char *outbuf = OPENSSL_malloc(IO_CHUNK + 32);
    int len = 0, rc;
    ...
    while (remaining > 0) {
        size_t chunk = remaining > IO_CHUNK ? IO_CHUNK : (size_t)remaining;
        if (fread(inbuf, 1, chunk, in) != chunk) die_msg("ciphertext read failed");
        if (EVP_DecryptUpdate(ctx, outbuf, &len, inbuf, (int)chunk) <= 0) die_ssl("DecryptUpdate failed");
        if (len > 0 && (size_t)fwrite(outbuf, 1, (size_t)len, out) != (size_t)len) die_msg("plaintext write failed");
        total += (uint64_t)len;
        remaining -= chunk;
    }

    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, GCM_TAG_LEN, (void *)tag) <= 0) die_ssl("SET_TAG failed");
    rc = EVP_DecryptFinal_ex(ctx, outbuf, &len);
    if (rc <= 0) die_msg("GCM authentication failed");
    if (len > 0 && (size_t)fwrite(outbuf, 1, (size_t)len, out) != (size_t)len) die_msg("final plaintext write failed");
    total += (uint64_t)len;
    if (total != expected_plain_len) die_msg("plaintext length mismatch");
...
static int cmd_encrypt(const char *pq_pub_path, const char *rsa_pub_path) {
    ...
    unsigned char aad[MAGIC_LEN + 4 + 4 + 8 + GCM_IV_LEN];
    ...
    memset(aad + 16, 0, 8);
    memcpy(aad + 24, iv, GCM_IV_LEN);

    fwrite(MAGIC, 1, MAGIC_LEN, stdout);
    write_u32_be(stdout, (uint32_t)pq_ct_len);
    write_u32_be(stdout, (uint32_t)rsa_ct_len);
    long plain_len_pos = ftell(stdout);
    write_u64_be(stdout, 0);
    fwrite(iv, 1, GCM_IV_LEN, stdout);
    fwrite(pq_ct, 1, pq_ct_len, stdout);
    fwrite(rsa_ct, 1, rsa_ct_len, stdout);

    gcm_encrypt_stream(stdin, stdout, aes_key, iv, aad, sizeof(aad), &plain_len, tag);

    if (fseek(stdout, plain_len_pos, SEEK_SET) != 0) die_msg("stdout seek failed; use regular file or seekable output for this version");
    write_u64_be(stdout, plain_len);
    if (fseek(stdout, 0, SEEK_END) != 0) die_msg("stdout seek end failed");
    fwrite(tag, 1, GCM_TAG_LEN, stdout);
...
static int cmd_decrypt(const char *pq_priv_path, const char *rsa_priv_path) {
    ...
    long current = ftell(stdin);
    if (current < 0) die_msg("stdin must be seekable for this version");
    if (fseek(stdin, 0, SEEK_END) != 0) die_msg("stdin seek end failed");
    long end = ftell(stdin);
    if (end < 0) die_msg("stdin ftell failed");
    if ((uint64_t)end < (uint64_t)current + GCM_TAG_LEN) die_msg("truncated input");
    cipher_len = (uint64_t)end - (uint64_t)current - GCM_TAG_LEN;
    if (fseek(stdin, end - GCM_TAG_LEN, SEEK_SET) != 0) die_msg("stdin seek tag failed");
    if (fread(tag, 1, GCM_TAG_LEN, stdin) != GCM_TAG_LEN) die_msg("short read on tag");
    if (fseek(stdin, current, SEEK_SET) != 0) die_msg("stdin seek ciphertext failed");
    ...
    aad[14] = (unsigned char)((rsa_ct_len >> 8) & 0xff);
    aad[15] = (unsigned char)(rsa_ct_len & 0xff);
    aad[16] = (unsigned char)((plain_len >> 56) & 0xff);
    aad[17] = (unsigned char)((plain_len >> 48) & 0xff);
    aad[18] = (unsigned char)((plain_len >> 40) & 0xff);
    aad[19] = (unsigned char)((plain_len >> 32) & 0xff);
    aad[20] = (unsigned char)((plain_len >> 24) & 0xff);
    aad[21] = (unsigned char)((plain_len >> 16) & 0xff);
    aad[22] = (unsigned char)((plain_len >> 8) & 0xff);
    aad[23] = (unsigned char)(plain_len & 0xff);
    memcpy(aad + 24, iv, GCM_IV_LEN);

    gcm_decrypt_stream(stdin, stdout, aes_key, iv, aad, sizeof(aad), cipher_len, tag, plain_len);
...
static void usage(void) {
    fprintf(stderr,
        ...
        "\n"
        "Note: this streaming version requires seekable output for encryption and seekable input for decryption.\n"
        "For huge regular files, use shell redirection to files, not anonymous pipes.\n");
}
