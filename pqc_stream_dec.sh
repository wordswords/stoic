...
usage() {
  cat >&2 <<'USAGE'
Usage:
  pqc_stream_dec.sh --pq-privkey pq_priv.pem --rsa-privkey rsa_priv.pem \
...
[ -n "$PQ_PRIV" ] || usage
[ -n "$RSA_PRIV" ] || usage
CMD=("$BIN" decrypt --pq-privkey "$PQ_PRIV" --rsa-privkey "$RSA_PRIV")

if [ "$DRY_RUN" -eq 1 ]; then
  if [ -n "$INPUT" ] && [ -n "$OUTPUT" ]; then
    [ "$CHECKSUM_IN_MODE" != "off" ] && echo "# input checksum mode: $CHECKSUM_IN_MODE for $(quote "$INPUT")"
    if [ "$PROGRESS" -eq 1 ]; then
      echo "pv -ptebar -s $(quote "$(file_size "$INPUT")") $(quote "$INPUT") | $(quote "$BIN") decrypt --pq-privkey $(quote "$PQ_PRIV") --rsa-privkey $(quote "$RSA_PRIV") > $(quote "$OUTPUT")"
    else
      echo "$(quote "$BIN") decrypt --pq-privkey $(quote "$PQ_PRIV") --rsa-privkey $(quote "$RSA_PRIV") < $(quote "$INPUT") > $(quote "$OUTPUT")"
    fi
    [ "$CHECKSUM_OUT_MODE" != "off" ] && echo "# output checksum mode: $CHECKSUM_OUT_MODE for $(quote "$OUTPUT")"
  else
    echo "$(quote "$BIN") decrypt --pq-privkey $(quote "$PQ_PRIV") --rsa-privkey $(quote "$RSA_PRIV")"
  fi
  exit 0
fi
